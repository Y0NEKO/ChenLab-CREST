...
#load all sample
{
  ...
  ogncell = read.csv("sc_result/CREST_final_20221204/pandaOGN_identity_group_V2_20221206.csv")
  ...
  for (i in 1:length(cellls)) {
    ...
    cellls[[i]]$Cell.type = cellls[[i]]$ident
  }
  ...
  library(qs)
  qsave(scrawls,"sc_result/CREST_final_20221204/scrawls_12_14.qs")
  # v1lsc = 
...
#blacklist select
{
  ...
  bl = qread("final_result/blacklist/x.blacklist_0.01_12_09_bulk_e11&bulk_sample.qs")
...
#Filter data
{
  ...
  FilterRecover = function(v1lsx,cell,thres,title){
    ...
    stat = data.frame("process" = c("common_BCraw","common_BC","common_array","common_with_cell"), 
                      "count" = c(length(comBCraw),length(comBC),length(comarray1),
  ...
  qsave(datals,file = "final_result/CREST_final_analysis/array_recover_data_total_12_14.qs")
...
#data build
{
  ...
  orderOGN = c('NbDA','DA','GLUFP','NbGLU','NbFP','GLUBM2','GLUBM1','NbBM1',
               'GABABL4','GABABL1','GABABL2','GLUAL1',
               'GABABI','GLUMHB2','NbGABABI','GABA_unk','Rgl1')
  ...
  for (i in 1:length(datals$arraydata_bl)) {
    ...
    heatmapls_order[[i]] = MyHeatmapCal(datals$arraydata_bl[[i]],names(datals$arraydata_bl)[i],
                                        orderls[[i]])
    names(heatmapls_order)[i] = names(heatmaplsv1_order)[i] = names(heatmaplsv2_order)[i] = names(datals$arraydata_bl)[i]
  }
  ...
  qsave(datals,file = "final_result/CREST_final_analysis/array_recover_data_total_withfigure_12_16.qs")
