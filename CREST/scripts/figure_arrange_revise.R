  ...
  #simulate control data
  {
    ...
    #hamming tree test
    {
      ...
      SplitHclustTreeRepCorHm = function(treels,nsample){
        ...
        # mx1 = CalCormx2(treels[[1]],length(treels[[1]]$labels))
        # mx2 = CalCormx2(treels[[2]],length(treels[[2]]$labels))
        # mx3 = CalCormx2(treels[[3]],length(treels[[3]]$labels))
        # mx1 = melt(mx1)
        # mx2 = melt(mx2)
        # mx3 = melt(mx3)
        # mxt = Reduce(function(x,y) merge(x,y,by = c("Var1","Var2")),list(mx1,mx2,mx3))
...
#fig3. E11 E15桑基图------------
{
  ...
  BCEnrichment2 = function(allu,cellorder = NULL){
    #绘制桑基图
    {
      phasestf = NULL
  ...
  for (i in 1:4) {
    ...
    cellallui = BCEnrichment2(cellallui)
...
#fig4. E11 E15 domain 分布图 & clone stat-----------
{
...
#fig6. OGN analysis
{
  ...
  ht_cluster = data.frame("celltype" = c("E11_NbGABA0","E11_NPBL","E11_NbGABABL","E11_NbGLUAL1",
                                         ...
                                         "OGN_NbDA","OGN_DA","OGN_NbGLU","OGN_GLUFP","OGN_NbFP"),
                          "domain" = rep(c("c1","c2","c3","c1","c2","c3"),c(4,3,2,6,3,5)))
  ...
  mycol_order = c("OGN_GABABL1","OGN_GABABL4","OGN_GABABL2","OGN_GLUAL1","OGN_GABABI","OGN_GLUMHB2",
                  "OGN_GLUBM2","OGN_GLUBM1","OGN_NbBM1",
                  "OGN_NbDA","OGN_DA","OGN_NbGLU","OGN_GLUFP","OGN_NbFP",
                  "OGN_NbGABABI","OGN_GABA_unk","OGN_Rgl1")
  #Heatmap
  OGNHeatmap = function(e11arydf,ognarydf,title)
  {
    count1 = e11arydf
    count2 = ognarydf
    ...
    mx = cor(cell_norm)
    dd = Dist(t(cell_norm), method = "abpearson")
    mx = as.matrix(1 - dd)
    for (i in 1:nrow(mx)) {
      mx[i,i] = 1
    }
  ...
  qsave(e11ognls,file = "final_result/CREST_final_analysis/OGN_analysis/pandaE11_OGNls.qs")
...
#sfig. replicates analysis----------
{
  ...
  RepCorCal = function(maxtrixls){
    ...
    if(nrow(repcorl) == 3){
      repcorl$cocor = sqrt( ( (repcorl$cor[1])^2 + (repcorl$cor[2])^2 + (repcorl$cor[3])^2 ) - 
                              ( 2 * repcorl$cor[1] * repcorl$cor[2] * repcorl$cor[3]) )
      p1.0 = ggscatter(mxt,x = "rep1",y = "rep2",size = 0.5) + ggtitle(paste0("cor = " ,round(cor(mxt$rep1,mxt$rep2),3)))
      p1.1 = ggscatter(mxt,x = "rep1",y = "rep3",size = 0.5) + ggtitle(paste0("cor = " ,round(cor(mxt$rep1,mxt$rep3),3)))
      p1.2 = ggscatter(mxt,x = "rep2",y = "rep3",size = 0.5) + ggtitle(paste0("cor = " ,round(cor(mxt$rep2,mxt$rep3),3)))
      pt = p1.0+p1.1+p1.2
    ...
    return(list("repcordf" = repcorl,"repcorpp" = pt))
  }
  repcorls = list()
  repcorlsv1 = list()
  repcorlsv2 = list()
  ...
  for (i in 1:length(maxtrixls)) {
    ...
    repcordf = rbind(repcordf,repcorls[[i]]$repcordf)
  ...
  p1.1 = ggbarplot(arrycordf[which(!arrycordf$sample_group %in% c("OGN","pandaE11")),],
                   x = "sample_group", y = "cor", color = "group",
                   width = 0.5,
                   add = c("mean_sd", "jitter"),
  ...
  ggexport(p1.0,filename = "final_result/CREST_final_analysis/0.qc/replicates_lineage_correlation.pdf",
           width = 5,height = 5)
  ggexport(p1.1,filename = "final_result/CREST_final_analysis/0.qc/array_type_correlation.pdf",
           width = 6,height = 5)
  
  datals$figure$replicates_cor = list("repcor_pointls" = repcorls,"repcor_barplot" = p1.0,
                                      "repcordf" = repcordf,"arraytypecordf" = p1.1,
...
#sfig2. basic eidt stat bulk---------
{
  ...
  #              list.files("bulk_result/X.bulk_in_vivo/bulk_20221125/",full.names = T)[-9])
  # v1lsx = lapply(paste0(samplels,"/v1/final_scarform.csv"), function(x) read.csv(x,header = T)) 
  # v2lsx = lapply(paste0(samplels,"/v2/final_scarform.csv"), function(x) read.csv(x,header = T)) 
  # names(v1lsx) = names(v2lsx) = c(list.files("bulk_result/X.bulk_in_vivo/bulk_20221125/previous/"),
  #                                 list.files("bulk_result/X.bulk_in_vivo/bulk_20221125/")[-9])
  # bulkls = list("v1" = v1lsx,"v2" = v2lsx)
  # qsave(bulkls,"bulk_result/X.bulk_in_vivo/bulk_total_12_9.qs")
  ...
  v2propft = EditArrayStat(v2lsx)
  tmp = v1lsx$e7_pos
  write.table(v1propft,file = "final_result/CREST_final_analysis/rebbutal_analysis/eidt_stat_v1_12_13_qc_intactnoNONE.txt",row.names = F,
              quote = F,sep = "\t")
  write.table(v2propft,file = "final_result/CREST_final_analysis/rebbutal_analysis/eidt_stat_v2_12_13_qc_intactnoNONE.txt",row.names = F,
              quote = F,sep = "\t")
...
#sfig5. sc sample v1 v2 recover stat & E11 E15 clone size stat
{
  ...
  #(1) recover stat
  {
    ...
    write.csv(recoverttls$recover_stat_sample,file = "final_result/CREST_final_analysis/rebbutal_analysis/recover_stat.csv",
              quote = F,row.names = F)
  }
  ...
  # clone cluster edit size stat
  {
    ...
    scbasicstls$clone_edit_numst = list("cluster" = list("E11" = p2.5.1,"E15" = p2.5.2),
                                        "domain" = list("E11" = p2.6.1,"E15" = p2.6.2))
  ...
  # edit type draw
  {
    ...
    for (i in 1:length(datals$arraydata_bl)) {
      editv1 = datals$arraydata_bl[[i]]$pattern.v1
      editv2 = datals$arraydata_bl[[i]]$pattern.v2
      ...
      # edit_stat2$log2_clone_size = log2(edit_stat2$clone_size)
      edit_statmd = edit_stat %>% group_by(edit_num) %>% 
        summarise(md = median(log2_clone_size))
      mkt = mk.test(edit_statmd$md)
      
      p2.4.1 = ggboxplot(data = edit_stat,x = "edit_num", y = "log2_clone_size") +
        annotate("text",x = 3, y = max(edit_stat$log2_clone_size),
                 label = paste0("Mann-Kendall trend test: p =", round(mkt$p.value,3))) +
        ggtitle(names(datals$arraydata_bl)[i])
      
      # p2.4.1 = ggstatsplot::ggbetweenstats(data = edit_stat,x = "edit_num", y = "log2_clone_size") + 
      #   ggtitle(paste0(names(datals$v1)[i],"V1"))
      # p2.4.2 = ggstatsplot::ggbetweenstats(data = edit_stat2,x = "edit_num", y = "log2_clone_size") + 
      #   ggtitle(paste0(names(datals$v1)[i],"V2"))
      arrayeditls[[i]] = p2.4.1
    ...
    ggsave(arrayeditls$E15,filename = "final_result/CREST_final_analysis/rebbutal_analysis/E15_edit_num_clone_size.pdf",
           width = 8,height = 6)
  ...
  # stat text data
  {
    ...
    write.table(blfiltst,file = "final_result/CREST_final_analysis/rebbutal_analysis/blacklistfilterst.txt",
  ...
  qsave(datals,file = "final_result/CREST_final_analysis/array_recover_data_total_withfigure_12_14_edit.qs")
}
...
#compare transcriptome&lineage
{
  #compare OGN & E15 correrlation
  {
    #combine OGN & E15 heatmap
    {
      ...
      pdf("final_result/CREST_final_analysis/rebbutal_analysis/E15_OGN_correlation_cmp_heatmap2.pdf",
          width = 8, height = 6)
    ...
    #combine OGN & E15 correlation lineage tree
    {
      ...
      pdf("final_result/CREST_final_analysis/rebbutal_analysis/hclust_tree_cmp.pdf",width = 8,height = 6)
    ...
    p1.0 = ggplot(cmplnmxl,aes(x = corr.e15, y = corr.ogn)) + geom_point() + 
      annotate("text",x = 0.05, y = max(cmplnmxl$corr.ogn),
               label = paste0("r = ", round(cor(cmplnmxl$corr.e15,cmplnmxl$corr.ogn),3),
                              ", p = ", round(t.test(cmplnmxl$corr.e15,cmplnmxl$corr.ogn,paired = T)$p.value,3))) +
      # ggrepel::geom_text_repel( data = cmplnmxl[which(cmplnmxl$corr.e15 > 0.1),], color = "red",
      #                           aes(label = paste0(Var1,"-",Var2))) +
      xlab("E15 lineage correlation of cell clusters") + ylab("OGN lineage correlation of cell clusters") +
      theme_pubr()
    p1.0
    ggsave(p1.0,filename = "final_result/CREST_final_analysis/rebbutal_analysis/E15_OGN_correlation_cmp.pdf",
           width = 6,height = 6)
    ...
    library(ggfortify)
    library(cluster)
    p1.1.1 = autoplot(pc, data = cmpclone, colour = 'sample',size = 0.5,frame = TRUE,frame.type = 'norm') + 
      scale_color_nejm()+
    ...
    p1.1.2 = p1.1.3 + plot_spacer() + p1.1.1 + p1.1.4 + 
      plot_layout(ncol = 2, nrow = 2, widths = c(4, 1), heights = c(1, 4))
    p1.1.2
    ggsave(p1.1.2,filename = "final_result/CREST_final_analysis/rebbutal_analysis/E15_OGN_clone_pca.pdf",
           width = 6,height = 6)
  ...
  #compare cell composition and transcriptome
  {
    ...
    library(ggExtra)
    library(gridExtra)
    pdf("final_result/CREST_final_analysis/rebbutal_analysis/OGN_E15_transcmp2.pdf",width = 8,height = 8)
    do.call("grid.arrange", c(p1.4, ncol=4))
    dev.off()
...
{
  #NbMP DA & Rgl1 DA
  ognda = qread("final_result/CREST_final_analysis/annotation_20221207/panda_DA_cell.BC.qs")
  ...
  p1.5 = ggplot(cmpdf,aes(x = NPBM,y = Rgl1)) + geom_point(size = 0.5) + 
    annotate("text",x = max(cmpdf$NPBM)/5, y = max(cmpdf$Rgl1),
             label = paste0("p = ", round(t.test(cmpdf$NPBM,cmpdf$Rgl1,paired = T)$p.value,3),
                            ", r = ", round(cor(cmpdf$NPBM,cmpdf$Rgl1),3)),
             size = 3) +
    ggtitle("Gene expression correlation") +
    theme_classic() + xlab("NPBM Derived DA") + ylab("Rgl1 Derived DA")
  p1.5
  ggsave(p1.5,filename = "final_result/CREST_final_analysis/rebbutal_analysis/OGN_NPBM-Rgl1_DA_transcmp_point.pdf",
         width = 5,height = 4)
  ...
  npbmda = qread("final_result/CREST_final_analysis/annotation_20221207/panda_NPBM_DA_fate.BC.qs")
  ...
  p1.6 = ggplot(cmpdf,aes(x = DA_NPBM,y = Other)) + geom_point(size = 0.5) + 
    annotate("text",x = max(cmpdf$DA_NPBM)/5, y = max(cmpdf$Other),
             label = paste0("p = ", round(t.test(cmpdf$DA_NPBM,cmpdf$Other,paired = T)$p.value,3),
                            ", r = ", round(cor(cmpdf$DA_NPBM,cmpdf$Other),3)),
             size = 3) +
    ggtitle("Gene expression correlation") +
    theme_classic() + xlab("NPBM DA potential") + ylab("Other NPBM")
  p1.6
  ggsave(p1.6,filename = "final_result/CREST_final_analysis/rebbutal_analysis/OGN_NPBM-DA_NPBM-other_transcmp_point.pdf",
         width = 5,height = 4)
  ...
  VocanoFigure(npdadeg,"DA fate vs non-DA fate NPBM",thres = 0.25)
